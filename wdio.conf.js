...
exports.config = {
  ...
  before: function (capabilities, specs) {
    ...
    //TODO lgandecki set this up based on the local env, so to not start two browsers if not needed
    global.browser.instances = [
      browserAlice, browserBob
    ];
    global.server = global.ddp = new DDP('http://127.0.0.1:3000').connect();


    var setupDdp = function () {
      console.log('[chimp][helper] setup DDP');
        try {
          console.log('[chimp][helper] connecting via DDP to', process.env['chimp.ddp0']);
          global.ddp.connectSync();
          addServerExecute(global.ddp);
          for(let i = 0; i < global.ddp.instances.length; i++) {
            console.log('[chimp][helper] connecting via DDP to ' + global.ddp.instances[i]._original.host + ':' + global.ddp.instances[i]._original.port);
            global.ddp.instances[i].connectSync();
            addServerExecute(global.ddp.instances[i]);
          }
          console.log('[chimp][helper] connecting via DDP had no error');
        } catch (error) {
